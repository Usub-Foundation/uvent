...
namespace usub::array::concurrent
{
    ...
    {
        // clz64 on purpose: `unsigned long` is 32-bit on Windows (LLP64), so the
        // clzl_portable() variant computed a bogus level there for every index >= 1.
        std::size_t L = (index == 0) ? 0 : static_cast<std::size_t>(63 - clz64(static_cast<uint64_t>(index) + 1));
    ...
    public:
        ...
        std::size_t emplace_back(Args&&... args)
        {
            ...
            if (c.state.compare_exchange_strong(st, Cell<T>::WRITING, std::memory_order_acq_rel,
        ...
        bool pop_back()
        {
            for (;;)
            {
                ...
                if (pub == 0)
                    return false;
                if (this->published_.compare_exchange_weak(pub, pub - 1, std::memory_order_acq_rel,
                                                           std::memory_order_acquire))
                {
                    ...
                    if (c.state.compare_exchange_strong(st, Cell<T>::DELETING, std::memory_order_acq_rel,
                                                        std::memory_order_acquire))
                    {
        ...
        void reserve(std::size_t n)
        {
            ...
            {
                if (this->buckets_[i].load(std::memory_order_acquire))
                    continue;
                if (!ensure_bucket(i))
                    break;
            }
    ...
    private:
        ...
        {
            if (this->buckets_[L].load(std::memory_order_acquire))
                return true;
            ...
            if (!this->buckets_[L].compare_exchange_strong(exp, cand, std::memory_order_acq_rel,
                                                           std::memory_order_acquire))
            {
        ...
        void advance_published() noexcept
        {
            for (;;)
            {
                ...
                if (!b0)
                    break;
                if (b0->cells[off0].state.load(std::memory_order_acquire) != Cell<T>::READY)
                    break;
                ...
                for (;;)
                {
                    ...
                    if (!b)
                        break;
                    if (b->cells[off].state.load(std::memory_order_acquire) != Cell<T>::READY)
                        break;
                    ++scan;
                }

                if (this->published_.compare_exchange_weak(cur, scan, std::memory_order_acq_rel,
                                                           std::memory_order_acquire))
                {
                    continue;
                }
        ...
        void shrink_published_tail_() noexcept
        {
            for (;;)
            {
                ...
                if (st == Cell<T>::READY || st == Cell<T>::WRITING || st == Cell<T>::DELETING)
                    return;
    ...
    private:
        alignas(usub::data_structures::metadata::CACHELINE_SIZE) std::atomic<std::size_t> alloc_idx_{0};

        alignas(usub::data_structures::metadata::CACHELINE_SIZE) std::atomic<std::size_t> published_{0};

        alignas(usub::data_structures::metadata::CACHELINE_SIZE) std::atomic<Bucket<T>*> buckets_[MAX_LEVELS]{};
    };
} // namespace usub::array::concurrent
